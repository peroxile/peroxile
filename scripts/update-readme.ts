...
function saveState(hash: string, word: string): void {
  fs.writeFileSync(
    STATE_FILE,
...
async function main(): Promise<void> {
  // FETCH DATA
  const repos = await api<Repo[]>(`/users/${owner}/repos?per_page=100&type=owner`);
  const sizeByLang: LangMap = {};

  for (const r of repos) {
    if (r.fork) continue;
    const langs = await api<LangMap>(
      `/repos/${owner}/${encodeURIComponent(r.name)}/languages`
    );
  ...
  const used = rows.reduce((a, [, v]) => a + v, 0);
